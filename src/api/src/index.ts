...
async function createCosmosClient() {
  try {
    ...
    if (!process.env.COSMOS_DB_DATABASE_NAME) {
      throw new Error('COSMOS_DB_DATABASE_NAME environment variable is required');
    }

    // Check if we're using the local emulator
    const isLocalEmulator = process.env.COSMOS_DB_ENDPOINT.includes('localhost') || 
    ...
    container = database.container(process.env.COSMOS_DB_CONTAINER_NAME || 'demo-container');
    ...
    if (error instanceof Error && error.message.includes('credentials')) {
      // eslint-disable-next-line no-console
      console.error('For local development, ensure you are logged in to Azure CLI: az login');
    }
...
app.get('/', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    const { resource: item } = await container.item(DEMO_ITEM.id, DEMO_ITEM.partitionKey).read();
