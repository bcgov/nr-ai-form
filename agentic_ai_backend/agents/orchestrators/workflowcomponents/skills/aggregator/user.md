...
6. **No useful response.** If both agents fail to provide valid content — neither responds, both return `Not found` / `no match`, or the only available content is an error message — direct the user to [FrontCounter BC](http://www.frontcounterbc.gov.bc.ca/).
