...
1.  **Start the server**:
    ...
    ```
2.  **Invoke via API**:
    Direct sub-agent calls must include `client_settings`. In normal production traffic, call the orchestrator instead so it resolves these settings from Cosmos DB and forwards them to the sub-agent.
    ...
    set `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY`, `AZURE_BLOBSTORAGE_CONNECTIONSTRING`, and `AZURE_BLOBSTORAGE_CONTAINER` before invoking directly in the .env.
