...
Direct calls to the Form Support Agent must include `client_settings`. Production callers should invoke the orchestrator, which resolves tenant settings from Cosmos DB and forwards this payload automatically.
...
set `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY`, `AZURE_BLOBSTORAGE_CONNECTIONSTRING`, and `AZURE_BLOBSTORAGE_CONTAINER` before invoking directly.
