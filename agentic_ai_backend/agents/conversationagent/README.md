...
Direct calls to the Conversation Agent must include `client_settings`. Browser callers should use the API backend gateway so tenant settings are resolved from Cosmos DB and forwarded automatically.
