...
if (clientInstance === 'ms') {
    ...
    (function () {
        ...
        const historyUrlBase = 'http://localhost:8003/history';
        ...
        async function loadHistory(sessionId) {
            try {
                const response = await fetch(`${historyUrlBase}/${sessionId}`);
                if (response.ok) {
                    const data = await response.json();
                    if (data && data.history) {
                        console.log("Conversation history loaded", data.history);
                        // Store the loaded history into local storage
                        localStorage.setItem('nrAiForm_conversationHistory', JSON.stringify(data.history.conversation_history || []));
                        // If chat UI is already initialized, we populate its history
                        if (document.getElementById('wp-chat-messages')) {
        ...
        function initWebSocket(sessionId) {
            ...
            socket.onerror = function(error) {
                ...
                if (pendingReject) {
                    pendingReject(new Error("WebSocket error"));
                    pendingResolve = null;
                    pendingReject = null;
                }
            };
        ...
        async function sendData(message, fieldData) {
            ...
            if (socket && socket.readyState === WebSocket.OPEN) {
                ...
                console.warn('WebSocket not connected, falling back to HTTP strategy...');
                try {
                    let data;
