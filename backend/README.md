...
The `/api/chat` endpoint accepts and returns several fields that orchestrate the conversational form-filling experience. Understanding the purpose of each field is crucial for proper integration.

#### Request Payload Fields

##### `user_message` (Required)
- **Type:** `string`
- **Purpose:** A text message entered by the user in the AI assistance form
- **Description:** This is the natural language input from the user that the AI agent analyzes to extract information and determine which form fields to populate
- **Example:** `"My name is John Smith and my email is john@example.com"`

##### `form_fields` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** All form fields available across the entire multi-page form or on the current webform page
- **Description:** Contains the most current data at the time of the payload request, including:
  - User-selected values
  - Values updated by the AI form assistant
  - Data loaded from the server or database
- **Structure:** Each field object contains properties like `data_id`, `fieldLabel`, `fieldType`, `fieldValue`, `is_required`, and `options`
- **Example:**
  ```json
  [
    {
...
##### `filled_fields` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** Data that has already been processed or populated by the agentic AI
- **Description:** For multi-page flows (e.g., POSSE), this is limited to the fields relevant to the current payload only. The agent receives this data along with `user_message` and may return updates for multiple fields at once
- **Usage:** Tracks which fields have been successfully populated during the conversation
- **Example:**
  ```json
  [
    {
...
- **Type:** `List[Dict]`
- **Purpose:** A list of fields that the agentic AI has identified as still needing values
- **Description:** Used internally by the agent logic to track incomplete form requirements. The AI uses this to determine what information to request from the user next
- **Example:**
  ```json
  [
    {
...
- **Type:** `List[Dict]`
- **Purpose:** The field the AI agent is actively working on
- **Description:** Used by the front end to:
  - Determine which field should be highlighted or focused
  - Display validation messages
  - Handle human-in-the-loop interactions
- **Constraint:** Only one field can be active at any time
- **Example:**
  ```json
  [
    {
...
- **Type:** `List[Dict]`
- **Purpose:** The full conversation history for the current session
- **Description:** Future versions of the agentic AI will use this to generate more context-aware or fine-tuned responses. Currently stores the back-and-forth dialogue between user and AI
- **Example:**
  ```json
  [
    {
    ...
    {
      "role": "assistant",
      "content": "Thanks! I've captured your name."
    }
  ]
  ```
...
##### `status` (Optional)
- **Type:** `string`
- **Purpose:** Indicates the current state of the agentic flow
- **Description:** This field is included for conversation-flow purposes but is not currently in active use
- **Possible Values:**
  - `"in_progress"` - The agent is actively processing information
  - `"awaiting_info"` - The agent is waiting for specific information from the user
  - `"completed"` - All required form fields have been filled

##### `thread_id` (Optional)
- **Type:** `string`
- **Purpose:** Reserved for future memory and context-linking capabilities
- **Description:** A unique identifier for the conversation thread. When provided, allows the agent to maintain context across multiple API calls. If not provided, a new thread_id will be generated
...
##### `thread_id` (Required)
- **Type:** `string`
- **Purpose:** Unique identifier for the conversation session
- **Description:** Return this value in subsequent requests to maintain conversation context

##### `response_message` (Required)
- **Type:** `string`
- **Purpose:** A message generated by the AI agent indicating what information it expects next or what action should occur
- **Description:** This is the natural language response that should be displayed to the user
- **Example:** `"Thanks John! I've captured your name. Could you provide your email address?"`

##### `status` (Required)
- **Type:** `string`
- **Purpose:** Current state of the form-filling workflow
- **Values:** Same as request `status` field

##### `filled_fields` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** Updated list of fields that have been successfully populated
- **Description:** May contain newly filled fields from the current interaction

##### `missing_fields` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** Updated list of fields still requiring values
- **Description:** Used to determine what to ask for next

##### `current_field` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** The field currently being processed
- **Description:** Frontend should highlight or focus this field

##### `conversation_history` (Optional)
- **Type:** `List[Dict]`
- **Purpose:** Updated conversation history including the latest exchange
